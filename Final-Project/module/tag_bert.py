...
class modified_bert(nn.Module):
    def __init__(self, model):
        ...
        self.tag_output = nn.Linear(200, 2)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels):
        ...
        span_logits = self.linear(sequence_output)
        span_logits = self.dropout2(span_logits)
        span_logits = self.tag_output(span_logits)
        ...
        for i, n in enumerate(start_target):
            if torch.isnan(n):
                start_target[i] = ignore_index
                end_target[i] = ignore_index

        loss_fct = CrossEntropyLoss(ignore_index=ignore_index)
        start_loss = loss_fct(start_logits, start_target.long())
        end_loss = loss_fct(end_logits, end_target.long())
